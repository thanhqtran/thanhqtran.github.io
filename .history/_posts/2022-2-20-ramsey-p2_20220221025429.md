...
In part 2, we deal with the continous-time version.
...
Terminologically speaking: $c(t)$ is the \textbf{control variable} since it can jump (taking any value at any point in time) while $k(t)$ is the \textbf{state variable} because it accumulates over time. ${\lambda}(t)$ is the co-state variable (also known as \textit{shadow price} in economics because it shows the marginal cost of violating the constraints at each point in time). 
